...
public class GestioneRicercheServlet extends Servlet {
	...
	private void ricercaUtenteNonLoggato(HttpServletRequest request,
		...
		try {
			...
			String stringa = "";
			if (mrequest.getParameter("stringa") != null) {
				stringa = mrequest.getParameter("stringa");
			}

			String citta = "";
			if (mrequest.getParameter("citta") != null) {
				citta = mrequest.getParameter("citta");
			}

			if (stringa.equals("") && citta.equals("")) {

				session.setAttribute("Errore",
						"Il campo tag o il campo città sono obbligatori.");
				redirect("GestioneRicercheServlet?to=redirectToPaginaRicerca",
						request, response);
				return;
			}
	...
	private void ricercaTag(HttpServletRequest request,
		...
		try {
			...
			if (stringa.equals("") && citta.equals("")) {
				...
				if (tok != null)
					redirect(
							"GestioneRicercheServlet?to=redirectToPaginaRicerca",
							request, response);
				else
					redirect(
							"GestioneRicercheServlet?to=redirectToPaginaRicercaUtenteNonLoggato",
							request, response);
			...
			if (mrequest.getParameter("data") != null
					&& !mrequest.getParameter("data").equals("")) {
