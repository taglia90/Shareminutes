...
public class GestioneUtentiServlet extends Servlet {
	...
	private void redirectToPaginaProfilo(HttpServletRequest request,
		...
		try {
			...
			int mioIdUtente = tok.getIdUtente();

			List<Prenotazione> listaClienti = gestionePrenotazioniRemote
					.getListaClienti(mioIdUtente, idUtente);
			List<Prenotazione> listaFornitori = gestionePrenotazioniRemote
					.getListaFornitori(mioIdUtente, idUtente);

			boolean isClienteOFornitore;
			if (listaFornitori == null || listaFornitori.isEmpty()
					|| listaClienti == null || listaClienti.isEmpty())
				isClienteOFornitore = true;
			else
				isClienteOFornitore = false;
			session.setAttribute("isClienteOFornitore", isClienteOFornitore);
	...
	public Long getNumeroDiFollowers(int idUtente) {
		...
		try {
			...
			GestioneUtentiRemote gestioneUtentiRemote = (GestioneUtentiRemote) PortableRemoteObject
					.narrow(ref1, GestioneUtentiRemote.class);
			numFollowers = gestioneUtentiRemote
					.getNumeroDiFollowers(idUtente);
