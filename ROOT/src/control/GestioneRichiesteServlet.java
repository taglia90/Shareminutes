...
public class GestioneRichiesteServlet extends Servlet {
	...
	private void redirectToPaginaDettaglioRichiesta(HttpServletRequest request,
		...
		try {
			...
			if (request.getParameter("idRichiesta") != null) {
				...
				if (richiesta.getLettoDa() != 0
						&& richiesta.getLettoDa() != tok.getIdUtente()) {
					gestioneRichiesteRemote.setRichiestaALetta(idRichiesta);
				}
	...
	public boolean ciSonoMessaggiNonLetti(int idUtente) {
		boolean isLetta = false;
		try {
			Context jndiContext = new javax.naming.InitialContext();
			Object ref1 = jndiContext.lookup("GestioneRichieste/remote");
			GestioneRichiesteRemote gestioneRichiesteRemote = (GestioneRichiesteRemote) PortableRemoteObject
					.narrow(ref1, GestioneRichiesteRemote.class);
			isLetta = gestioneRichiesteRemote
					.ciSonoMessaggiNonLettiDiUtente(idUtente);
	...
	public List<Messaggio> recuperaListaMessaggi(int idRichiesta) {

	List<Messaggio> listaMessaggi = new ArrayList<Messaggio>();
		try {
			...
			GestioneRichiesteRemote gestioneRichiesteRemote = (GestioneRichiesteRemote) PortableRemoteObject
				...
				listaMessaggi = gestioneRichiesteRemote
						.getListaMessaggiDiRichiesta(idRichiesta);
